...
if __name__ == "__main__":
    parser = train_arg_parser("Train AT-D3QN (Pan et al. 2023) flipper policy inside FTR-Benchmark (Isaac Sim)", play=True)
...
# ============================================================
# BLOCK 3 — Config dataclass for FTR-specific D3QN training
# ============================================================

@dataclass
class FtrD3QNConfig:
    """Minimal AT-D3QN config for FTR-Benchmark training (no physics engine fields).

    Mirrors FtrPPOConfig's env/run/physics fields so D3QN and PPO configs stay
    structurally similar, but replaces the on-policy PPO/GAE fields with the
    off-policy replay-buffer / target-network / epsilon-greedy fields that
    Double-Dueling DQN training actually needs.
    """
...
if __name__ == "__main__":
    ...
    # FtrD3QNConfig is built here only to read the env fields build_ftr_gym_env needs.
    # env_cfg_overrides must set module_name: atd3qn so FtrEnv routes observations
    # and rewards through ATD3QNModule instead of the default marv_rl one.
    _cfg = FtrD3QNConfig(**raw_cfg)
