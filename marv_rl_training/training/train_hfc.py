...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train HFC (Azayev & Zimmermann 2022) flipper policy inside FTR-Benchmark (Isaac Sim)")
    parser.add_argument("--config", type=str, required=True, help="Path to an HFC config yaml (see FtrHFCConfig)")
...
from rl_modules.hfc.hfc_policy import HFCPolicy


# ============================================================
# BLOCK 3 — Config dataclass for FTR-specific HFC training
# ============================================================

@dataclass
class FtrHFCConfig:
    """Minimal HFC config for FTR-Benchmark training (no physics engine fields).

    Structurally identical to FtrD3QNConfig (AT-D3QN, train_d3qn.py) — same off-policy
    replay-buffer / target-network / epsilon-greedy Double-Dueling DQN loop. The
    difference is entirely in HFCPolicy/HFCModule: the Q-network selects one of 7 named
    poses (Azayev & Zimmermann 2022, Fig. 1) rather than a flipper-angle delta, and the
    policy decodes the selected pose into a per-step delta command toward that pose's
    fixed target angle.
    """
    ...
    policy_opts: dict[str, Any]         # HFCPolicy kwargs (track_vel, hm_hidden, hm_out, fusion_hidden,
                                         # front_up_deg, front_down_deg, back_up_deg, back_down_deg, ...)
    ...
    # Must include `module_name: hfc` so FtrEnv computes HFC's 18-D obs/reward
    # (see rl_modules/registry.py) and FtrTorchRLEnv picks the matching HFCObservation.
...
# ============================================================
# BLOCK 4 — FtrHFCTrainer
# ============================================================

class FtrHFCTrainer:
    """Off-policy Double-Dueling DQN trainer for the Hybrid Flipper Controller (HFC),
    adapted from Azayev & Zimmermann 2022. Structurally identical to FtrD3QNTrainer
    (train_d3qn.py) — same env setup, RunLogger, crash-recovery, replay buffer + target
    network + epsilon-greedy loop.

    The paper itself trains its state-transition classifier via imitation learning from
    human demonstrations (cross-entropy loss against demonstrated state labels); we have
    no such demonstrations in this automated Isaac Sim pipeline, so this trainer instead
    trains the pose-selection Q-network via reward-driven Double DQN, using
    HFCModule.get_reward_components() (this project's own adaptation — the paper defines
    no RL reward at all).
    """

    def __init__(self, raw_config: "DictConfig", ftr_gym_env: gymnasium.Env, optuna_trial=None):
        self.optuna_trial = optuna_trial
        self.config = FtrHFCConfig(**raw_config)
        ...
        self.term_logger = get_terminal_logger("ftr_hfc_train")
        ...
        # ---- policy (Q-network + epsilon-greedy pose selection + template decode) ----
        self.policy = HFCPolicy(epsilon=self.config.epsilon_start, **self.config.policy_opts).to(self.device)
        ...
        self._grad_steps = 0
        self.term_logger.info("Initialized FtrHFCTrainer.")

    # ------------------------------------------------------------------
    # Crash recovery — mirrors FtrD3QNTrainer, only the Q-network needs
    # saving/loading (no separate value head / actor-critic split).
    # ------------------------------------------------------------------
    ...
    def _dqn_update(self, batch) -> dict[str, float]:
        ...
        q_values = self.policy.q_network(obs)  # (B, 7)
    ...
    def _train(self):
        ...
        pbar = tqdm(total=self.config.total_frames, desc="FTR HFC Training", unit="frames", leave=False)
    ...
    def _get_eval_rollout_results(self) -> dict[str, float]:
        self.env.eval()
        self.policy.eval()  # disables epsilon-greedy exploration (see HFCPolicy.forward)
...
if __name__ == "__main__":
    ...
    # Must set module_name: hfc here (via env_cfg_overrides in the yaml) so FtrEnv
    # routes observations/rewards through HFCModule instead of the default marv_rl one.
    ...
    if args.play is not None:
        ...
        policy = HFCPolicy(epsilon=0.0, **_cfg.policy_opts).to(_cfg.device)
        ...
        trainer = FtrHFCTrainer(raw_cfg, ftr_gym_env)
        trainer.train()
