...
if __name__ == "__main__":
    parser = train_arg_parser("Train CREPS (Pecka et al. 2016) flipper policy inside FTR-Benchmark (Isaac Sim)", play=True)
...
if __name__ == "__main__":
    ...
    # FtrCREPSConfig is built here only to read the env fields build_ftr_gym_env needs.
    # env_cfg_overrides must set module_name: creps so FtrEnv routes observations
    # and rewards through CREPSModule instead of the default marv_rl one.
    ...
    if args.play is not None:
        ...
        run_trainer(FtrCREPSTrainer, raw_cfg, ftr_gym_env)
